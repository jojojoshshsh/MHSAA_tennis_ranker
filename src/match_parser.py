...
def parse_bracket_matches(bracket_data: dict, event_id, event_date: str,
    ...
    for item in items:
        ...
        # Only process played matches (score present)
        ...
        # Determine winner by isWinner flag
        ...
        # Scores are already from winner's perspective: "6 - 4" = winner 6, loser 4
        set_score = _parse_bracket_score(score_raw)

        # Prefer matchId (links to school-report dedup); fall back to bracket slot UUID
...
def extract_school_meta(data: dict) -> dict:
    ...
    return {"id": school.get("id"), "name": school.get("name", ""),
            "division_boys": div_boys, "division_girls": div_girls}
