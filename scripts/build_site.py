...
for entry in team_data:
    ...
    df     = entry["df"]
    thead = "<thead><tr>" + "".join(f'<th onclick="sortTable(this)">{col}</th>' for col in df.columns) + "</tr></thead>"
    tbody = "<tbody>" + "".join(
...
all_data = []
# Also build a master list of all rows for school search
all_rows_for_search = []
...
# Collect all unique school names for autocomplete
...
# Build per-file full data index for school search
csv_full_data = {}
for csv_path in sorted(src_dir.glob("*.csv")):
    if csv_path.stem.startswith("team_"):
        continue
...
function makeAutocomplete(inputId, listId, onSelect) {{
  ...
  function updateList(val) {{
    ...
    list.innerHTML = matches.map(s =>
      `<div onclick="selectSchool('${{inputId}}','${{listId}}','${{s.replace(/'/g,"\\'")}}', onSelect_${{inputId}})">${{s}}</div>`
    ).join('');
  }}
  return updateList;
}}

// Global select handlers (named to work with inline onclick)
...
function doSchoolSearch(school) {{
  ...
  // Find all matching rows across all CSV files
  const byFlight = {{}};
  ...
  for (const [label, data] of Object.entries(byFlight).sort()) {{
    const thead = '<thead><tr>' + data.cols.map(c => `<th onclick="sortTable(this)">${{c}}</th>`).join('') + '</tr></thead>';
    const tbody = '<tbody>' + data.rows.map(r =>
      '<tr class="highlight-row">' + r.map(v => `<td>${{v}}</td>`).join('') + '</tr>'
    ).join('') + '</tbody>';
    results.innerHTML += `
      <div style="margin-bottom:1.5rem;">
        <h3 style="font-size:.95rem;color:#1a3a5c;margin-bottom:.5rem;">${{label}}</h3>
        <div class="table-wrap"><table class="rankings-table">${{thead}}${{tbody}}</table></div>
      </div>`;
  }}
}}
...
function runCompare() {{
  ...
  for (const key of allKeys) {{
    const ea = dataA[key];
    const eb = dataB[key];
    html += `<div class="compare-flight"><h3>${{key}}</h3><div class="compare-cols">`;

    for (const [label, entry, other] of [[a, ea, eb],[b, eb, ea]]) {{
      html += `<div class="compare-col"><h4>${{label}}</h4>`;
      if (!entry) {{
        ...
        for (const col of SHOW_COLS) {{
          ...
          html += `<div class="compare-stat">
            <span>${{col}}</span>
            <span class="${{better ? 'compare-winner' : ''}}">${{v}}</span>
          </div>`;
        }}
      }}
      html += `</div>`;
...
print(f"Built docs/index.html with {len(all_data)} sections")
